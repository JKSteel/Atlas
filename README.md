...
That's it. The EXIF tool used to read GPS data from your media (`exiftool-vendored`) is bundled automatically by npm.
...
Output is written to `out/`. You can then package it with `electron-builder` or distribute the `out/` folder directly.
...
atlas/
├── assets/
│   └── textures/         globe texture images (you provide)
├── EXTRA/
│   └── cities500.txt     GeoNames dataset (you provide)
├── media/                your photos and videos (you provide)
│   └── CategoryName/
├── data/
│   └── pins.json         auto-generated by the scanner, do not edit
├── src/
